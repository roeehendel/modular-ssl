...
def main():
    ...
    method = get_method(encoder, args)
    datamodule = get_datamodule(args)
    attach_pretrain_transform(datamodule, method, args)
...
def get_arg_parser() -> ArgumentParser:
    ...
    parser.add_argument("--encoder_type", type=str, default="resnet", help="The encoder type to use",
                        choices=encoders_list)

    # SSL Method
    parser = SSLMethod.add_argparse_args(parser)
    ...
    parser = add_encoder_args(parser, temp_args)
...
def add_encoder_args(parser: ArgumentParser, temp_args) -> ArgumentParser:
    encoder_class: Type[Encoder] = encoders.registry.get(temp_args.encoder_type)
    parser = encoder_class.add_argparse_args(parser)
    return parser
...
def get_encoder(args) -> Encoder:
    encoder_constructor: Type[Encoder] = encoders.registry.get(args.encoder_type)
    encoder = encoder_constructor(**vars(args))
    args.encoder_full_name = encoder.full_name()
    return encoder


def get_method(encoder: Encoder, args) -> SSLMethod:
    method_constructor: Type[SSLMethod] = ssl_methods.registry.get(args.method)
    method_lit_module = method_constructor(encoder, **vars(args))
...
def attach_pretrain_transform(datamodule: SSLDataModule, method: SSLMethod, args):
    pretrain_transform = method.branches_views_transform(normalization=datamodule.normalization_transform)
    datamodule.pretrain_transform = pretrain_transform


def get_loggers(args):
    experiment_name = f"{args.method}-{args.encoder_full_name}-{args.dataset}"
...
def get_callbacks(method: SSLMethod, args):
    ...
    embed_dim = method.encoder.embedding_dim()
