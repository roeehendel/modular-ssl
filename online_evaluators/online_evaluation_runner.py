...
class OnlineEvaluationRunner(Callback):
    ...
    def __init__(self, embedding_dim: int, online_evaluators: list[OnlineEvaluator]):
        super().__init__()
    ...
    def on_train_batch_end(self, trainer: Trainer, pl_module: LightningModule,
                           outputs: Sequence, batch: Sequence, batch_idx: int, **kwargs) -> None:
        self._create_and_save_embeddings(pl_module, batch, batch_idx, stage='train')
    ...
    def on_validation_batch_end(self, trainer: Trainer, pl_module: LightningModule,
        ...
        self._create_and_save_embeddings(pl_module, batch, batch_idx, stage='val')
    ...
    def _create_and_save_embeddings(self, pl_module: LightningModule, batch: Sequence, batch_idx: int,
                                    stage: str) -> None:
        if stage == 'train':
            views, labels = batch
            branches_views, eval_view = views
        else:
            eval_view, labels = batch
        embeddings = self._get_embeddings(pl_module, eval_view)
    ...
    @staticmethod
    def _get_embeddings(pl_module, eval_view):
        with torch.no_grad():
            with set_training(pl_module, False):
                embeddings = pl_module.encoder(eval_view)
        return embeddings
